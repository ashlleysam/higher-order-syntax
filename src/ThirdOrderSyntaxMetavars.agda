...
module ThirdOrderSyntaxMetavars
  -- Kinds
  (Knd : Set)
  -- Kind of types
  (* : Knd)
...
module Syntax1
  ...
  --                  Pos₁ c Γ2 (renVec ξ ts) .fst ≡ renBinders ξ (Pos₁ c Γ1 ts .fst))
  ...
  ⟦Pos₁⟧ : (s : Shape₁) (Γ : Ctx) → TyVec Γ (Pos₀₁ s) → List (Σ[ Γ' ∈ Ctx ] (Ctx1 (Γ' ++ Γ) × Typ (Γ' ++ Γ))) × Typ Γ
  fst (⟦Pos₁⟧ s Γ η) = map (interpBinders Γ η) (Pos₁ s .fst)
  snd (⟦Pos₁⟧ s Γ η) = interpTm (Pos₁ s .snd) η IdSub

  subVecPos₁ : ∀{Γ1 Γ2 s} (σ : Sub Γ1 Γ2) (η : TyVec Γ1 (Pos₀₁ s)) →
                ⟦Pos₁⟧ s Γ2 (subVec σ η) .snd ≡ sub σ (⟦Pos₁⟧ s Γ1 η .snd)
  subVecPos₁ {Γ1} {Γ2} {s} σ η = interpTmSub (Pos₁ s .snd) η σ IdSub IdSub (Id◦ σ ∙ sym (◦Id σ))
 
  renVecPos₁ : ∀{Γ1 Γ2 s} (ξ : Ren Γ1 Γ2) (η : TyVec Γ1 (Pos₀₁ s)) →
               ⟦Pos₁⟧ s Γ2 (renVec ξ η) .snd ≡ ren ξ (⟦Pos₁⟧ s Γ1 η .snd)
  renVecPos₁ {Γ1} {Γ2} {s} ξ η =
    ⟦Pos₁⟧ s Γ2 (renVec ξ η) .snd
      ≡⟨ sym (cong (λ x → ⟦Pos₁⟧ s Γ2 x .snd) (subVecι ξ η)) ⟩
    ⟦Pos₁⟧ s Γ2 (subVec (ι ξ) η) .snd
      ≡⟨ subVecPos₁ (ι ξ) η ⟩
    sub (ι ξ) (⟦Pos₁⟧ s Γ1 η .snd)
      ≡⟨ subι ξ  (⟦Pos₁⟧ s Γ1 η .snd) ⟩
    ren ξ (⟦Pos₁⟧ s Γ1 η .snd) ∎

  subVecCtxPos₁ : ∀{Γ1 Γ2 s} (σ : Sub Γ1 Γ2) (η : TyVec Γ1 (Pos₀₁ s)) →
                   ⟦Pos₁⟧ s Γ2 (subVec σ η) .fst ≡ subBinders σ (⟦Pos₁⟧ s Γ1 η .fst)
  subVecCtxPos₁ {Γ1} {Γ2} {s} σ η =
    map (interpBinders Γ2 (subVec σ η)) (Pos₁ s .fst)
      ≡⟨ map-cong (interpBinders∘subVec σ η) (Pos₁ s .fst) ⟩
    map (subBinder σ ∘ interpBinders Γ1 η) (Pos₁ s .fst)
      ≡⟨ map-compose (Pos₁ s .fst) ⟩
    map (subBinder σ) (map (interpBinders Γ1 η) (Pos₁ s .fst)) ∎

  renVecCtxPos₁ : ∀{Γ1 Γ2 s} (ξ : Ren Γ1 Γ2) (η : TyVec Γ1 (Pos₀₁ s)) →
                  ⟦Pos₁⟧ s Γ2 (renVec ξ η) .fst ≡ renBinders ξ (⟦Pos₁⟧ s Γ1 η .fst)
  renVecCtxPos₁ {Γ1} {Γ2} {s} ξ η =
    ⟦Pos₁⟧ s Γ2 (renVec ξ η) .fst
      ≡⟨ cong (λ x → ⟦Pos₁⟧ s Γ2 x .fst) (sym (subVecι ξ η)) ⟩
    ⟦Pos₁⟧ s Γ2 (subVec (ι ξ) η) .fst
      ≡⟨ subVecCtxPos₁ (ι ξ) η ⟩
    subBinders (ι ξ) (⟦Pos₁⟧ s Γ1 η .fst)
      ≡⟨ subBindersι ξ (⟦Pos₁⟧ s Γ1 η .fst) ⟩
    renBinders ξ (⟦Pos₁⟧ s Γ1 η .fst) ∎
  ...
  data Tm Γ Δ where
    ...
    constr1 : (s : Shape₁) →
              (ts : TyVec Γ (Pos₀₁ s)) →
              (es : TmVec Γ Δ (⟦Pos₁⟧ s Γ ts .fst)) →
              Tm Γ Δ (⟦Pos₁⟧ s Γ ts .snd)
