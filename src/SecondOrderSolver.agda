...
data Exp : (A : TyRep) → Set where
  -- Pure values
  ιExp : ∀{A} (x : ⟦ A ⟧) → Exp A

  -- Function application
  EApp : ∀{A B} (f : Exp (Fun A B)) (a : Exp A) → Exp B

  -- Renamings
  ERenε : Exp (Ren' [] [])
  ERenKeep : ∀{Γ1 Γ2 t} → Exp (Ren' Γ1 Γ2) → Exp (Ren' (t ∷ Γ1) (t ∷ Γ2))
  ERenDrop : ∀{Γ1 Γ2 t} → Exp (Ren' Γ1 Γ2) → Exp (Ren' Γ1 (t ∷ Γ2))
  ERenId : ∀{Γ} → Exp (Ren' Γ Γ)
  _•E_ : ∀{Γ1 Γ2 Γ3} → Exp (Ren' Γ2 Γ3) → Exp (Ren' Γ1 Γ2) → Exp (Ren' Γ1 Γ3)
  EKeep* : ∀{Γ1 Γ2} → Exp (Ren' Γ1 Γ2) → ∀ Δ → Exp (Ren' (Δ ++ Γ1) (Δ ++ Γ2))
  EDrop* : ∀{Γ1 Γ2} → Exp (Ren' Γ1 Γ2) → ∀ Δ → Exp (Ren' Γ1 (Δ ++ Γ2))

  -- Substitutions
  ESubε : ∀{Γ} → Exp (Sub' [] Γ)
  _▸E_ : ∀{Γ1 Γ2 t} → Exp (Sub' Γ1 Γ2) → Exp (Tm' Γ2 t) → Exp (Sub' (t ∷ Γ1) Γ2)
  ESubId : ∀{Γ} → Exp (Sub' Γ Γ)
  _•◦E_ : ∀{Γ1 Γ2 Γ3} → Exp (Ren' Γ2 Γ3) → Exp (Sub' Γ1 Γ2) → Exp (Sub' Γ1 Γ3)
  _◦•E_ : ∀{Γ1 Γ2 Γ3} → Exp (Sub' Γ2 Γ3) → Exp (Ren' Γ1 Γ2) → Exp (Sub' Γ1 Γ3)
  _◦E_ : ∀{Γ1 Γ2 Γ3} → Exp (Sub' Γ2 Γ3) → Exp (Sub' Γ1 Γ2) → Exp (Sub' Γ1 Γ3)
  ESubKeep* : ∀{Γ1 Γ2} → Exp (Sub' Γ1 Γ2) → ∀ Δ → Exp (Sub' (Δ ++ Γ1) (Δ ++ Γ2))
  ιE : ∀{Γ1 Γ2} → Exp (Ren' Γ1 Γ2) → Exp (Sub' Γ1 Γ2)

  -- Variables
  V0 : ∀{Γ t} → Exp (Var' (t ∷ Γ) t)
  VS : ∀{Γ s t} → Exp (Var' Γ t) → Exp (Var' (s ∷ Γ) t)
  renEVar : ∀{Γ1 Γ2 t} → Exp (Ren' Γ1 Γ2) → Exp (Var' Γ1 t) → Exp (Var' Γ2 t)

  -- Terms
  varE : ∀{Γ t} → Exp (Var' Γ t) → Exp (Tm' Γ t)
  constrE : ∀{Γ} (s : ⅀ .TyShape) (es : Exp (TmVec' Γ (⅀ .TyPos s .fst))) → Exp (Tm' Γ (⅀ .TyPos s .snd))
  renETm : ∀{Γ1 Γ2 t} → Exp (Ren' Γ1 Γ2) → Exp (Tm' Γ1 t) → Exp (Tm' Γ2 t)
  subETm : ∀{Γ1 Γ2 t} → Exp (Sub' Γ1 Γ2) → Exp (Tm' Γ1 t) → Exp (Tm' Γ2 t)
  subEVar : ∀{Γ1 Γ2 t} → Exp (Sub' Γ1 Γ2) → Exp (Var' Γ1 t) → Exp (Tm' Γ2 t)
  ...
  _∷E_ : ∀{Γ Δ t Σ} →
        ...
        (es : Exp (TmVec' Γ Σ)) →
        Exp (TmVec' Γ ((Δ , t) ∷ Σ))
  renETmVec : ∀{Γ1 Γ2 Σ} → Exp (Ren' Γ1 Γ2) → Exp (TmVec' Γ1 Σ) → Exp (TmVec' Γ2 Σ)
  subETmVec : ∀{Γ1 Γ2 Σ} → Exp (Sub' Γ1 Γ2) → Exp (TmVec' Γ1 Σ) → Exp (TmVec' Γ2 Σ)

  -- Raw renamings
  EURenε : Exp URen'
  EURenKeep : Exp URen' → Exp URen'
  EURenDrop : Exp URen' → Exp URen'
  EURenId : Exp URen'
  _•U_ : Exp URen' → Exp URen' → Exp URen'
  EUKeep* : Exp URen' → Exp ℕ' → Exp URen'
  EUDrop* : Exp URen' → Exp ℕ' → Exp URen'
  untyERen' : ∀{Γ1 Γ2} → Exp (Ren' Γ1 Γ2) → Exp URen'

  -- Raw substitutions
  EUSubε : Exp USub'
  _▸U_ : Exp USub' → Exp UTm' → Exp USub'
  EUSubId : Exp USub'
  _•◦U_ : Exp URen' → Exp USub' → Exp USub'
  _◦U'_ : Exp USub' → Exp USub' → Exp USub'
  untyESub : ∀{Γ1 Γ2} → Exp (Sub' Γ1 Γ2) → Exp USub'

  -- Raw variables/natural numbers
  Z : Exp ℕ'
  S : Exp ℕ' → Exp ℕ'
  renEUVar : Exp URen' → Exp ℕ' → Exp ℕ'
  untyEVar : ∀{Γ t} → Exp (Var' Γ t) → Exp ℕ'

  -- Raw terms
  varU : Exp ℕ' → Exp UTm'
  constrU : (s : ⅀ .TyShape) (es : Exp UTmVec') → Exp UTm'
  renEUTm : Exp URen' → Exp UTm' → Exp UTm'
  subEUTm : Exp USub' → Exp UTm' → Exp UTm'
  subEUVar : Exp USub' → Exp ℕ' → Exp UTm'
  untyETm : ∀{Γ t} → Exp (Tm' Γ t) → Exp UTm'

  -- Raw term vectors
  []U : Exp UTmVec'
  _∷U_ : (ek : Exp UTm' × Exp ℕ') →
         (es : Exp UTmVec') →
         Exp UTmVec'
  renEUTmVec : Exp URen' → Exp UTmVec' → Exp UTmVec'
  subEUTmVec : Exp USub' → Exp UTmVec' → Exp UTmVec'
  untyETmVec : ∀{Γ Σ} → Exp (TmVec' Γ Σ) → Exp UTmVec'
...
-- Apply Keep
simplKeep : ∀{Γ1 Γ2 t} (ξ : Exp (Ren' Γ1 Γ2)) → Σ[ ξ' ∈ Exp _ ] interp ξ' ≡ Keep {t = t} (interp ξ)
simplKeep ERenId = ERenId , refl
simplKeep ξ = ERenKeep ξ , refl

-- Fuse two renamings
simpl•≡ : ∀{Γ1 Γ2 Γ2' Γ3} (ξ1 : Exp (Ren' Γ2 Γ3)) (ξ2 : Exp (Ren' Γ1 Γ2')) →
          (p : Γ2' ≡ Γ2) →
          Σ[ ξ' ∈ Exp (Ren' Γ1 Γ3) ] interp ξ' ≡ interp ξ1 • subst (Ren Γ1) p (interp ξ2)
...
simpl•≡ {Γ1} {Γ2} {Γ2'} {Γ3} ξ1 ξ2 p = 
  J (λ Γ2 p → (ξ1 : Exp (Ren' Γ2 Γ3)) (ξ2 : Exp (Ren' Γ1 Γ2')) →
      Σ[ ξ' ∈ Exp (Ren' Γ1 Γ3) ] interp ξ' ≡ interp ξ1 • subst (Ren Γ1) p (interp ξ2))
    (λ ξ1 ξ2 → (ξ1 •E ξ2) , refl)
    Γ2 p ξ1 ξ2

simpl• : ∀{Γ1 Γ2 Γ3} (ξ1 : Exp (Ren' Γ2 Γ3)) (ξ2 : Exp (Ren' Γ1 Γ2)) →
            Σ[ ξ' ∈ Exp _ ] interp ξ' ≡ interp ξ1 • interp ξ2
simpl• ξ1 ξ2 = simpl•≡ ξ1 ξ2 refl

-- Rename a variable
simplRenVar≡ : ∀{Γ1 Γ1' Γ2 t} (ξ : Exp (Ren' Γ1 Γ2)) (x : Exp (Var' Γ1' t)) →
              (p : Γ1' ≡ Γ1) →
              Σ[ x' ∈ Exp (Var' Γ2 t) ] interp x' ≡ renVar (interp ξ) (subst (flip Var t) p (interp x))
...
simplRenVar≡ {Γ1} {Γ1'} {Γ2} {t} ξ x p =
    J (λ Γ1 p → (ξ : Exp (Ren' Γ1 Γ2)) (x : Exp (Var' Γ1' t)) →
      Σ[ x' ∈ Exp (Var' Γ2 t) ] interp x' ≡ renVar (interp ξ) (subst (flip Var t) p (interp x)))
      (λ ξ x → renEVar ξ x , refl)
      Γ1 p ξ x

simplRenVar : ∀{Γ1 Γ2 t} (ξ : Exp (Ren' Γ1 Γ2)) (x : Exp (Var' Γ1 t)) →
          Σ[ x' ∈ Exp _ ] interp x' ≡ renVar (interp ξ) (interp x)
simplRenVar ξ x = simplRenVar≡ ξ x refl

-- Convert a renaming to a substitution
simplι : ∀{Γ1 Γ2} (ξ : Exp (Ren' Γ1 Γ2)) → Σ[ σ ∈ Exp _ ] interp σ ≡ ι (interp ξ)
simplι ERenε = ESubε , refl
simplι ERenId = ESubId , refl
simplι ξ = ιE ξ , refl

-- Rename a term
simplRen : ∀{Γ1 Γ2 t} (ξ : Exp (Ren' Γ1 Γ2)) (e : Exp (Tm' Γ1 t)) →
         Σ[ e' ∈ Exp _ ] interp e' ≡ ren (interp ξ) (interp e)
-- Rename a vector of terms
simplRenVec : ∀{Γ1 Γ2 Σ} (ξ : Exp (Ren' Γ1 Γ2)) (es : Exp (TmVec' Γ1 Σ)) →
            Σ[ es' ∈ Exp _ ] interp es' ≡ renVec (interp ξ) (interp es)
-- Rename a substitution
simpl•◦ : ∀{Γ1 Γ2 Γ3} (ξ : Exp (Ren' Γ2 Γ3)) (σ : Exp (Sub' Γ1 Γ2)) →
          Σ[ σ' ∈ Exp _ ] interp σ' ≡ interp ξ •◦ interp σ
...
simpl•◦≡ : ∀{Γ1 Γ2 Γ2' Γ3} (ξ : Exp (Ren' Γ2 Γ3)) (σ : Exp (Sub' Γ1 Γ2')) →
           (p : Γ2' ≡ Γ2) → Σ[ σ' ∈ Exp _ ] interp σ' ≡ interp ξ •◦ subst (Sub Γ1) p (interp σ)
...
simpl•◦≡ {Γ1} {Γ2} {Γ2'} {Γ3} ξ σ p =
  J (λ Γ2 p → (ξ : Exp (Ren' Γ2 Γ3)) (σ : Exp (Sub' Γ1 Γ2')) →
    Σ[ σ' ∈ Exp (Sub' Γ1 Γ3) ] interp σ' ≡ interp ξ •◦ subst (Sub Γ1) p (interp σ))
    (λ ξ σ → ξ •◦E σ , refl)
    Γ2 p ξ σ
...
simpl◦•≡ : ∀{Γ1 Γ2 Γ2' Γ3} (σ : Exp (Sub' Γ2 Γ3)) (ξ : Exp (Ren' Γ1 Γ2')) →
           (p : Γ2' ≡ Γ2) →
           Σ[ σ' ∈ Exp _ ] interp σ' ≡ interp σ ◦• subst (Ren Γ1) p (interp ξ)
...
simpl◦•≡ {Γ1} {Γ2} {Γ2'} {Γ3} σ ξ p =
  J (λ Γ2 p → (σ : Exp (Sub' Γ2 Γ3)) (ξ : Exp (Ren' Γ1 Γ2')) →
    Σ[ σ' ∈ Exp (Sub' Γ1 Γ3) ] interp σ' ≡ interp σ ◦• subst (Ren Γ1) p (interp ξ))
    (λ σ ξ → σ ◦•E ξ , refl)
    Γ2 p σ ξ

simpl◦• : ∀{Γ1 Γ2 Γ3} (σ : Exp (Sub' Γ2 Γ3)) (ξ : Exp (Ren' Γ1 Γ2)) →
          Σ[ σ' ∈ Exp _ ] interp σ' ≡ interp σ ◦• interp ξ
simpl◦• σ ξ = simpl◦•≡ σ ξ refl

-- Substitute a variable
simplSubVar : ∀{Γ1 Γ2 t} (σ : Exp (Sub' Γ1 Γ2)) (x : Exp (Var' Γ1 t)) →
              Σ[ x' ∈ Exp _ ] interp x' ≡ subVar (interp σ) (interp x)
-- Substitute a term
simplSub : ∀{Γ1 Γ2 t} (σ : Exp (Sub' Γ1 Γ2)) (e : Exp (Tm' Γ1 t)) →
         Σ[ e' ∈ Exp _ ] interp e' ≡ sub (interp σ) (interp e)
-- Substitute a vector of terms
simplSubVec : ∀{Γ1 Γ2 Σ} (σ : Exp (Sub' Γ1 Γ2)) (es : Exp (TmVec' Γ1 Σ)) →
              Σ[ es' ∈ Exp _ ] interp es' ≡ subVec (interp σ) (interp es)
-- Fuse two substitutions              
simpl◦ : ∀{Γ1 Γ2 Γ3} (σ1 : Exp (Sub' Γ2 Γ3)) (σ2 : Exp (Sub' Γ1 Γ2)) →
         Σ[ σ' ∈ Exp _ ] interp σ' ≡ interp σ1 ◦ interp σ2

simplSubVar≡ : ∀{Γ1 Γ1' Γ2 t} (σ : Exp (Sub' Γ1 Γ2)) (x : Exp (Var' Γ1' t)) →
               (p : Γ1' ≡ Γ1) → Σ[ x' ∈ Exp _ ] interp x' ≡ subVar (interp σ) (subst (flip Var t) p (interp x))
...
simplSubVar≡ {Γ1} {Γ1'} {Γ2} {t} σ x p =
  J (λ Γ1 p → (σ : Exp (Sub' Γ1 Γ2)) (x : Exp (Var' Γ1' t)) →
    Σ[ e ∈ Exp (Tm' Γ2 t) ] interp e ≡ subVar (interp σ) (subst (flip Var t) p (interp x)))
...
simpl◦ σ1 ESubε = ESubε , refl
simpl◦ σ1 (σ2 ▸E e) with simpl◦ σ1 σ2 | simplSub σ1 e
... | (σ' , p) | (e' , q) = σ' ▸E e' , cong₂ _▸_ p q
simpl◦ σ1 ESubId = σ1 , sym (◦Id (interp σ1))
simpl◦ σ1 (ξ •◦E σ2) with simpl◦• σ1 ξ
... | (σ' , p) with simpl◦ σ' σ2
... | (σ'' , q) = 
  σ'' , 
  q
...
simpl◦ σ1 (σ2 ◦E σ3) with simpl◦ σ1 σ2
... | (σ' , p) with simpl◦ σ' σ3
... | (σ'' , q) = 
  σ'' , 
  q
...
simpl◦ σ1 (ιE ξ) with simpl◦• σ1 ξ
... | (σ' , p) =
  σ' ,
  p
  ∙ sym (◦ι (interp σ1) (interp ξ))
simpl◦ σ1 σ2 = σ1 ◦E σ2 , refl
...
-}
simpl : ∀{A} (e : Exp A) → Σ[ e' ∈ Exp A ] interp e' ≡ interp e
simpl (ιExp x) = ιExp x , refl
...
simplFun : ∀{A} → Exp A → Exp A
simplFun e = simpl e .fst

simpl≡ : ∀{A} (e : Exp A) → interp (simplFun e) ≡ interp e
simpl≡ e = simpl e .snd

module _ where
  example : ∀{Γ s t} (e : Tm Γ s) (v : Tm Γ t) →
      sub (IdSub ▸ v) (ren (Drop IdRen) e) ≡ e
  example e v = sym (simpl≡ (subETm (ESubId ▸E ιExp v) (renETm (ERenDrop ERenId) (ιExp e))))